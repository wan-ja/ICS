                ...
                retrofitAPI.createRental(rentalRequestDto).enqueue(new Callback<SuccessResponseDto>() {
                    ...
                    public void onResponse(Call<SuccessResponseDto> call, Response<SuccessResponseDto> response) {
                        if (response.isSuccessful()) {
                            Toast.makeText(AddActivity.this, response.body().getItemData().getMessage(), Toast.LENGTH_SHORT).show();
                        }
                        // response 400~500 (에러 발생)
                            ...
                            try {
                                ...
                                Toast.makeText(AddActivity.this, jsonObjError.getJSONObject("result").getString("msg"), Toast.LENGTH_LONG).show();
                            } catch (IOException e) {
                    ...
                    public void onFailure(Call<SuccessResponseDto> call, Throwable t) {
                        Toast.makeText(AddActivity.this, "통신에 실패하였습니다.", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        });
